...
Model buildToyModel(const fs::path modelPath) {
    ...
    LayerParams conv2_weightParam(sizeof(fp32), {5, 5, 32, 32}, modelPath / "conv2_weights.bin"); //not sure if the dimensions are right
    ...
    LayerParams mpl1_inDataParam(sizeof(fp32), {56, 56, 32});
    LayerParams mpl1_outDataParam(sizeof(fp32), {28, 28, 32});
    ...
    LayerParams conv3_weightParam(sizeof(fp32), {3, 3, 32, 64}, modelPath / "conv3_weights.bin"); //not sure if the dimensions are right
    ...
    LayerParams conv4_weightParam(sizeof(fp32), {3, 3, 64, 64}, modelPath / "conv4_weights.bin"); //not sure if the dimensions are right
    ...
    LayerParams mpl2_inDataParam(sizeof(fp32), {24, 24, 64});
    LayerParams mpl2_outDataParam(sizeof(fp32), {12, 12, 64});
    ...
    LayerParams conv5_weightParam(sizeof(fp32), {3, 3, 64, 64}, modelPath / "conv5_weights.bin"); //not sure if the dimensions are right
    ...
    LayerParams conv6_weightParam(sizeof(fp32), {3, 3, 64, 128}, modelPath / "conv6_weights.bin"); //not sure if the dimensions are right
    ...
    LayerParams mpl3_inDataParam(sizeof(fp32), {8, 8, 128});
    LayerParams mpl3_outDataParam(sizeof(fp32), {4, 4, 128});

    MaxPoolingLayer* mpl3 = new MaxPoolingLayer(mpl3_inDataParam, mpl3_outDataParam);
    model.addLayer(mpl3);
