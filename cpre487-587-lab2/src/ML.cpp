...
Model buildToyModel(const fs::path modelPath) {
    ...
    /*
     * Works up to here :) (compare w/layer 8 output)
     */
    ...
    LayerParams dense0_weightParam(sizeof(fp32), {2048, 256}, modelPath / "dense1_weights.bin");
    LayerParams dense0_biasParam(sizeof(fp32), {256}, modelPath / "dense1_biases.bin");
    ...
    LayerParams dense1_weightParam(sizeof(fp32), {256, 200}, modelPath / "dense2_weights.bin");
    LayerParams dense1_biasParam(sizeof(fp32), {200}, modelPath / "dense2_biases.bin");
    ...
    // Input shape: 200
    // Output shape: 200
    LayerParams smax0_inDataParam(sizeof(fp32), {200});
    LayerParams smax0_outDataParam(sizeof(fp32), {200});

    SoftmaxLayer* smax0 = new SoftmaxLayer(smax0_inDataParam, smax0_outDataParam);
    model.addLayer(smax0);

    //std::cout << "DEBUG: All 12 model layers added :]" << std::endl;
...
int main(int argc, char **argv) {
    ...
    // Construct a LayerData object from a LayerParams one
    //NOTE: YOU NEED TO CHANGE THE DIMENSIONS AS WELL AS THE FILE PATH
    //      USE L<X> as layer number for file path
    LayerData expected( { sizeof(fp32), {200}, basePath / "image_1_data" / "layer_11_output.bin" } );
